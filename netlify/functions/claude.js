exports.handler = async function(event) {
  ...
  const API_KEY = process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_API_KEY || process.env.mckinney_api_key;
  if (!API_KEY) return { statusCode: 500, headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ error: { message: 'API key not configured. Add ANTHROPIC_API_KEY in Netlify environment variables.' } }) };
  try {
